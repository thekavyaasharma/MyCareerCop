...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Idempotent: if already marked applied, don't double-count.
    if (jobs[idx].applied) {
      return res.status(200).json({ jobs, alreadyApplied: true });
    }
    ...
    return res.status(500).json({ error: "Failed to mark job as applied. Please try again." });
  }
}